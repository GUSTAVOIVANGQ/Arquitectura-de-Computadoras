-- ===============================
-- MEMORIA DE DATOS - VERSIÓN CORREGIDA
...
architecture Behavioral of DataMemory is
    ...
    -- Inicialización con algunos datos de prueba para load/store
    signal memory : mem_array := (
        ...
        4 => x"00000005",  -- x1 = 5 (para test)
        5 => x"00000003",  -- x2 = 3 (para test)
        6 => x"00000008",  -- x3 = 8 (para test)
        7 => x"0000000C",  -- x4 = 12 (para test)
        others => x"00000000"
    );
...
begin
    ...
    begin
        if MemRead = '1' then
            ...
            case funct3 is
                when "000" => -- LB (Load Byte con extensión de signo)
                    case byte_offset is
                        when 0 => 
                            if word_data(7) = '1' then
                                read_data <= x"FFFFFF" & word_data(7 downto 0); -- 24 + 8 = 32 bits
                            else
                                read_data <= x"000000" & word_data(7 downto 0); -- 24 + 8 = 32 bits
                            end if;
                        when 1 => 
                            if word_data(15) = '1' then
                                read_data <= x"FFFFFF" & word_data(15 downto 8); -- 24 + 8 = 32 bits
                            else
                                read_data <= x"000000" & word_data(15 downto 8); -- 24 + 8 = 32 bits
                            end if;
                        when 2 => 
                            if word_data(23) = '1' then
                                read_data <= x"FFFFFF" & word_data(23 downto 16); -- 24 + 8 = 32 bits
                            else
                                read_data <= x"000000" & word_data(23 downto 16); -- 24 + 8 = 32 bits
                            end if;
                        when 3 => 
                            if word_data(31) = '1' then
                                read_data <= x"FFFFFF" & word_data(31 downto 24); -- 24 + 8 = 32 bits
                            else
                                read_data <= x"000000" & word_data(31 downto 24); -- 24 + 8 = 32 bits
                            end if;
                ...
                when "001" => -- LH (Load Halfword con extensión de signo)
                    if byte_offset = 0 then
                        if word_data(15) = '1' then
                            read_data <= x"FFFF" & word_data(15 downto 0); -- 16 + 16 = 32 bits
                        else
                            read_data <= x"0000" & word_data(15 downto 0); -- 16 + 16 = 32 bits
                        end if;
                    elsif byte_offset = 2 then
                        if word_data(31) = '1' then
                            read_data <= x"FFFF" & word_data(31 downto 16); -- 16 + 16 = 32 bits
                        else
                            read_data <= x"0000" & word_data(31 downto 16); -- 16 + 16 = 32 bits
                        end if;
                    else
                ...
                when "010" => -- LW (Load Word)
                    read_data <= word_data; -- 32 bits directos
                    
                when "100" => -- LBU (Load Byte sin extensión de signo)
                    case byte_offset is
                        when 0 => read_data <= x"000000" & word_data(7 downto 0);   -- 24 + 8 = 32 bits
                        when 1 => read_data <= x"000000" & word_data(15 downto 8);  -- 24 + 8 = 32 bits
                        when 2 => read_data <= x"000000" & word_data(23 downto 16); -- 24 + 8 = 32 bits
                        when 3 => read_data <= x"000000" & word_data(31 downto 24); -- 24 + 8 = 32 bits
                        when others => read_data <= x"00000000";
                    end case;
...
-- ===============================
-- LB/LBU:  x"FFFFFF" (24 bits) + byte (8 bits) = 32 bits ✓
-- LH/LHU:  x"FFFF" (16 bits) + halfword (16 bits) = 32 bits ✓
-- LW:      word_data (32 bits) directo = 32 bits ✓
-- ===============================
